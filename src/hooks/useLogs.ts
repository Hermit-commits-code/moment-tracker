...
export function useLogs() {
  ...
  const saveLog = (data: any) => {
    // FIX: Force the date key to match the user's selected local date
    const timestamp = data.timestamp || new Date().toISOString();
    const dateKey = timestamp.split("T")[0];

    setLogs((prevLogs) => {
      const updatedLogs = JSON.parse(JSON.stringify(prevLogs)) as DailyLog[];
      const dayIndex = updatedLogs.findIndex((l) => l.date === dateKey);
      ...
      // Prepare Caffeine if it exists in this form submission
      ...
      if (dayIndex > -1) {
        ...
        // Add Caffeine to the day's total list
        if (newCaff) {
  ...
  const deleteLog = (id: string, date: string) => {
    setLogs((prev) => {
      const updated = prev
        ...
        .filter(
          (log) =>
            log.moodEntries.length > 0 || (log.caffeineEntries?.length || 0) > 0
        );
